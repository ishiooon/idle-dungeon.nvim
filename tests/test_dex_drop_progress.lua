...
local function find_enemy_label(items, name)
  for _, item in ipairs(items or {}) do
    if item.id == "dex_entry" and item.kind == "enemy" and (item.label or ""):match(name) then
...
local label1 = find_enemy_label(items1, "Tux Penguin")
...
local label2 = find_enemy_label(items2, "Tux Penguin")
