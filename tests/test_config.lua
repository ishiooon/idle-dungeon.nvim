...
-- 経験値の既定値は中盤以降の進行テンポを維持する値である。
assert_true(built.battle.reward_exp == 8, "経験値の既定値が定義通りである")
...
assert_true(built.ui.menu.height_ratio == 0.52, "メニューの既定高さ比率が設定される")
assert_true(built.ui.menu.min_width == 56, "メニューの既定最小幅が設定される")
assert_true(built.ui.menu.min_height == 18, "メニューの既定最小高さが設定される")
assert_true(built.ui.menu.max_width == 84, "メニューの既定最大幅が設定される")
...
assert_true(built.ui.menu.detail_preview == nil, "未使用の詳細プレビュー設定は持たない")
