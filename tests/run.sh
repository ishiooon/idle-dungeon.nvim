...
# メニュー上部の撃破アイコン表示が右下表示と一致することを確認する。
lua tests/test_menu_live_header_defeat_icon.lua
...
# メニュー本文の左クリックで行選択が切り替わることを確認する。
lua tests/test_menu_tabs_mouse_row_click.lua
...
# サブメニュー下部にもEnter説明が表示されることを確認する。
lua tests/test_menu_submenu_enter_hint_footer.lua
# サブメニューで非実行行のEnterがno-opで閉じないことを確認する。
lua tests/test_menu_submenu_enter_noop_no_close.lua
# メインメニューの横幅拡張と折り返し無効を確認する。
lua tests/test_menu_tabs_width_expand.lua
# 画面に余白がある場合はメニュー高さを拡張して後半項目を表示することを確認する。
lua tests/test_menu_compact_height_expand.lua
# タブ更新時にメニューの表示サイズが縮まないことを確認する。
lua tests/test_menu_tabs_stable_layout.lua
# ジョブ選択メニューがEnterで詳細遷移する前提の設定で開くことを確認する。
lua tests/test_menu_job_keep_open.lua
# ジョブ選択画面が1カラム要約表示になることを確認する。
lua tests/test_menu_job_compare_view.lua
# ジョブ選択メニューの詳細に成長情報と習得情報が表示されることを確認する。
lua tests/test_menu_job_detail_preview.lua
# ジョブ詳細の適用行で実際にジョブ変更できることを確認する。
lua tests/test_menu_job_detail_apply.lua
...
# 装備補正がスロットと希少度で差別化されることを確認する。
lua tests/test_player_equipment_balance.lua
