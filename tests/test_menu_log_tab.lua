-- このテストはログ行の上限を9999行に保ち、メニューにlogタブが表示されることを確認する。
...
for index = 1, 10005 do
  state = game_log.append(state, string.format("LOG_%05d", index))
end

assert_true(type(state.logs) == "table", "状態にログ配列が保持される")
assert_true(#state.logs == 9999, "ログは最大9999行に制限される")
assert_true(state.logs[1] == "LOG_00007", "ログが上限を超えると古い行から破棄される")
assert_true(state.logs[#state.logs] == "LOG_10005", "最新行が末尾に追加される")
...
for _, item in ipairs((log_tab and log_tab.items) or {}) do
  if item.id == "log_entry" then
    ...
    if label:match("LOG_10005") then
      latest_found = true
    end
    if label:match("LOG_00001") then
      oldest_trimmed = false
    end
  end
end
assert_true(entry_count == 9999, "logタブには最大9999件のログを表示する")
